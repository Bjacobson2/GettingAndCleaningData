...
names(dataTest) <-features[,2] #adding correct colnames from feature
names(dataTrain)<-features[,2] #adding correct colnames from feature
...
dataTest$Activity[dataTest$Activity == 6] <- "laying" #this could be done programmatically better with if/indices, but running low on time
...
#part 5 subsetting for averages and grouped by each variable 
averageData <-finalData %>% group_by(Activity) %>% summarise_each(funs(mean))
View(averageData)

#writing to file for submission
write.table(averageData,file="../Practice/partFive.txt",row.name=F)